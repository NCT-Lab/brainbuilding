...
def create_riemannian_classifier():
    """Create a KNN classifier using Riemannian geometry for covariance matrices"""
    from pyriemann.classification import KNearestNeighbor
    
    # Create classifier with Riemannian distance
    classifier = KNearestNeighbor(n_neighbors=int(KNN_NEIGHBORS), 
                                 metric='riemann',
                                 n_jobs=-1)
...
def evaluate_model_with_cv(X, y, subject_ids):
    ...
    for fold_idx, (train_idx, test_idx) in tqdm(enumerate(cv.split(X, y, subject_ids)), 
        ...
        classifier = create_riemannian_classifier()
...
def plot_fold_results(results_df, output_path):
    ...
    plt.ylabel('Score')
    plt.title('Riemannian KNN Performance Metrics by Subject (Leave-One-Subject-Out CV)')
    ...
    for metric in metric_cols:
        ...
        ax.set_title(f'{metric.capitalize()} Score by Subject (Riemannian KNN)')
        ...
        plt.tight_layout()
        metric_plot_path = os.path.join(os.path.dirname(output_path), f'riemannian_knn_subject_cv_{metric}.png')
...
def main():
    # Load the transformed covariance matrices
    ...
    # Plot results
    plot_path = os.path.join(output_dir, f'riemannian_knn_subject_cv_results.png')
    plot_fold_results(results['fold_results'], plot_path)
    
    # Save detailed results
    results_path = os.path.join(output_dir, f'riemannian_knn_subject_cv_results.pkl')
