...
output_dir = os.path.join(os.path.dirname(__file__), 'results-tuned')
...
def evaluate_model_with_cv(X_preprocessed, y, subject_ids, selected_subjects):
    ...
    for fold_idx, (train_idx, test_idx) in tqdm(enumerate(cv.split(X_subset, y_subset, subject_ids_subset)), 
        ...
        # Apply the calibrated threshold to the evaluation set
        ...
        tn, fp, fn, tp = confusion_matrix(y_eval, y_eval_pred).ravel()
        ...
        for metric, value in fold_metrics.items():
            if metric not in ['subject_id', 'num_samples', 'tp', 'fp', 'tn', 'fn', 'threshold']:
