...
from .transformers import ParallelTransportTransformer, TangentSpaceProjector, SubjectWhiteningTransformer
...
def evaluate_subject_with_riemannian_knn(X, y, event_ids, k=5, verbose=False):
    ...
    for i in tqdm(range(n_samples), desc="Evaluating samples", disable=not verbose):
...
def evaluate_pipeline_with_adaptation(
    ...
    for name, step in pipeline_steps:
        if isinstance(step, (ParallelTransportTransformer, TangentSpaceProjector)):
            pt_transformer = step
            break
    ...
    covariance_matrix_shape = covariance_transformer.fit_transform(X['sample'][0][np.newaxis, :, :])[0].shape
    # Convert all samples to covariance matrices upfront
    X_cov = np.zeros(len(X), dtype=[
    ...
    for train_idx, test_idx in tqdm(cv.split(X_cov, y, subject_ids), 
        ...
        segments = np.array([original_samples[:, :, i*250:(i+1)*250] for i in range(3)])
        
        # Reshape to get the desired order: (n_samples*3, 18, 250)
        X_test_reduced = segments.transpose(1, 0, 2, 3).reshape(n_samples*3, 18, 250)
        
        # Convert all samples to covariance matrices upfront
        X_test_cov = np.zeros(len(X_test_reduced), dtype=[
            ('sample', np.float64, covariance_matrix_shape),
            ('subject_id', np.int64),
            ('event_id', np.int64),
            ('is_background', np.int64)
        ...
        # Convert all samples to covariance matrices
        X_test_cov['sample'] = covariance_transformer.fit_transform(X_test_reduced)
        X_test = X_test_cov
        ...
        pt_transformer.set_subject_means_and_counts(temp_subject_means, temp_subject_counts)
        ...
        for i in np.where(test_mask)[0]:
            ...
            pt_transformer.partial_fit(X_test[i:i+1], custom_weights=np.array([1/3]))
