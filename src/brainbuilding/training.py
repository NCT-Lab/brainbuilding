...
def main():
    ...
    X = compute_normalized_augmented_covariances(X, order=ORDER)
    ...
    for df in [log_features_df, full_features_df]:
        ...
        for idx, sample_idx in enumerate(cv_predictions["sample_indices"]):
            df.loc[sample_idx, "cv_predicted_label"] = cv_predictions[
                "predicted_labels"
            ][idx]
            df.loc[sample_idx, "cv_proba_class_0"] = cv_predictions[
                "predicted_probas"
            ][idx][0]
            df.loc[sample_idx, "cv_proba_class_1"] = cv_predictions[
                "predicted_probas"
            ][idx][1]
